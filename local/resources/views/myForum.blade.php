...
<div class="logobg1"></div>
      ...
      <div class="col-md-12 padding25 bgw">
          ...
          <div class="col-md-6">
              ...
              <a href="{{ url()->previous()}}" class="btn btn-inverse">กลับสู่หน้ากระดาน</a>
              </div>
              <br>
          <div class="col-md-6">
            @php
              ...
              <div class="row click09" style="float: right;">
                <?php
                    if($page==1){?>
                      ...
                      <a href="{{url('Category/'.Session::get('userData')->user_type.'/'.$id.'/'.'1')}}" class="btn btn-success" style="margin-bottom: 15px;">&laquo;</a>
                      <?php
                  }
                  $p=0;
                    for ($i=1; $i <= $total_page; $i++) {
                      ...
                      if($total_page<4){
                      if($page==$i){?>
                        <a href="{{url('Category/'.Session::get('userData')->user_type.'/'.$id.'/'.$i)}}" class="btn btn-success active" style="margin-bottom: 15px;">{{$i}}</a>
                      <?php  }else{  ?>
                      <a href="{{url('Category/'.Session::get('userData')->user_type.'/'.$id.'/'.$i)}}" class="btn btn-success" style="margin-bottom: 15px;">{{$i}}</a>
                    <?php
                    }
                  }else{
                    if($page1>0 and $page2<=$total_page){
                    if($page==$i){?>
                    <a href="{{url('Category/'.Session::get('userData')->user_type.'/'.$id.'/'.$i)}}" class="active" style="margin-bottom: 15px;">{{$i}}</a>
                  <?php  }else if($i>=$page1 and $i<=$page2){  ?>
                    <a href="{{url('Category/'.Session::get('userData')->user_type.'/'.$id.'/'.$i)}}" style="margin-bottom: 15px;">{{$i}}</a>
                  <?php
                  }
                }else if($page2>$total_page){
                  $page1=$page1+($total_page-$page2);
                  if($page==$i){?>
                    <a href="{{url('Category/'.Session::get('userData')->user_type.'/'.$id.'/'.$i)}}" class="active btn btn-success" style="margin-bottom: 15px;">{{$i}}</a>
                ...
                $p++;
                    ...
                    if($page==$total_page){?>
                      ...
                      <a href="{{url('Category/'.Session::get('userData')->user_type.'/'.$id.'/'.$total_page)}}" class="btn btn-success" style="margin-bottom: 15px;">&raquo;</a>
                    <?php
                    }
                    ?>
  ...
  <div class="panel-body">
      ...
      @foreach ($forumsData as $forum)
        ...
        <div class="tr">
            ...
            <div class="td"><a href="{{url('detailForum/'.$forum->id)}}">{!!$forum->category_name!!}</a></div>
            ...
            <div class="td"><a href="{{url('detailForum/'.$forum->id)}}">{{$forum->user_fullname}}</a></div>
